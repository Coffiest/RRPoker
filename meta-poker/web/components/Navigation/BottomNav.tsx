...
const navItems = [
  { href: '/lobby',       label: 'ロビー',      icon: HomeIcon },
  { href: '/handhistory', label: 'ハンド',       icon: HistoryIcon },
...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-[#0A0A0A]/95 backdrop-blur-xl border-t border-[#1E1E1E] safe-area-pb">
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center gap-1 px-4 py-1 rounded-xl transition-all duration-150 ${
                active ? 'text-[#F2A900]' : 'text-[#383838] hover:text-[#606060]'
              }`}
            >
...
function HomeIcon({ className, active }: { className?: string; active?: boolean }) {
  return (
    <svg className={className} fill={active ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 0 : 1.5}>
